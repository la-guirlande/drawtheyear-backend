...
export default class AuthenticationService extends Service {
     ...
     * 
     * A token must be provided in the request header `x-access-token`. If the token is valid, the user is stored into `res.locals.user`.
     * 
    ...
    public async authenticateHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        if (token != null) {
            try {
                ...
                const user = await this.container.db.users.findById(data.userId).populate('emotions').populate('days.emotions');
                if (user != null) {
